 ...
 */
export async function validateNoProtectedRoles(roleIds: string[]): Promise<void> {
  const { Role } = await import('../models');
  const roles = await Role.findAll({
    where: { id: { [require('sequelize').Op]: roleIds } },
    attributes: ['id', 'name'],
  });
  ...
  if (protectedFound.length > 0) {
    ...
    throw new Error(`Cannot assign protected role(s): ${protectedNames}. These roles are reserved for system use only.`);
  }
}
